...
\usepackage{graphicx}
\pagestyle{fancy}
\lstset{language=[ANSI]C,frame=single,captionpos=b,flexiblecolumns=true,xleftmargin=20mm,xrightmargin=20mm,tabsize=4}
\lstset{morekeywords={bool,size\_t,uint32\_t,uint16\_t,uint8\_t,memmgr\_t,areamgr\_t,eqsbmgr\_t,blkmgr\_t}}
...
Podobnie jak w przypadku \verb+eqsbmgr+ manadżer bloków średniej wielkości
...
początku innego zarządzanego obszaru, wtedy łączy się wszytkie trzy; jeśli nie
...
manadżer pamięci ogólnego przeznaczenia nie posiada tak prostej implementacji.
...
zwolnieniu, o ile przylegają do wolnych --- może być nieporządaną cechą
manadżera pamięci. Zauważmy, że aplikacja korzystająca z alokatora może mieć w
...
w pojedyńczej linii.  Zauważmy również, że w obecnej implementacji rekord do
...
nadmiarowe dane, które nie są używane w trakcie obliczeń. Możnaby jednak zmusić
...
%%% Ładna tabelka - może się przydać %%%
...
przypisanych obszarowi wywołując procedure \verb+areamgr_shrink_area+,
...
Ostatnim elementem całej implementacji jest ogólny manadżer, który łączy
wszystkie dotychczas opisan elementy tworząc jeden uniwersalny interfejs
...
manadżerów tj. \verb+eqsbmgr+, \verb+blkmgr+ oraz \verb+mmapmgr+, i jest
tworzony w pierwszym nowo utworzonym obszarze, który jednocześnie jest
kontenerem dla manadżera \verb+areamgr+. 
...
przyległym do początku obszaru umieszcza rekord \verb+memmgr_t+ i inicjalizuje
jego zawartość,
...
przekazywane do podległego manadżera, a następnie sprawdza się czy nie powstały
...
%dwupoziomowość --- rozbić listę nieużytków na jedną globalną i wiele lokalnych
%na potrzeby alokatorów niższego rzędu.  Oba rodzaje list mają różne kryteria
