...
przeplatających się, operujące na blokach o tym samym rozmiarze.
...
kompilatory. W bieżącej implementacji przyjęto rozmiar wielkości $8$ bajtów.
...
może się zmieniać, natomiast nie może ona przekraczać 1024 bajtów.
...
W nagłówku superbloku znajdują się 16--bitowe wskaźniki listy, których
wykorzystanie zostanie omówione w ramach opisu manadżera superbloków. Oprócz
tego składowane są tam następujące informacje:
\begin{itemize}
...
pamięci podręcznej pokrywającej nagłówek i bitmapę. Stanowi to podstawę dla
szybkości alokatora korzystającego z superbloków.
...
zawsze z czterech superbloków składowanych na jednej stronie pamięci. Na
następnym rysunku grupy są otoczone grubą czarną ramką. O superblokach mówimy,
że są przyległe wtedy i tylko wtedy, gdy przylegają do siebie w obrębie jednej
grupy. Superblok zajęty różni się od używanego tym, iż nie posiada wolnych
bloków.
...
zarządzanego obszaru, zmniejszając rozmiar ostatniego superbloku. Manadżer
superbloków posiada następujące pola w swoim rekordzie\footnote{Pierwsze dwie
pozycje widać na rysunku, reszta została ukryta dla zachowania czytelności.}:
\begin{itemize}
...
\begin{itemize}
\item \verb+sb_mgr_init+ --
\item \verb+sb_mgr_alloc+ --
\item \verb+sb_mgr_free+ --
\item \verb+sb_mgr_add+ --
\item \verb+sb_mgr_expand+ --
\end{itemize}
...
\begin{itemize}
\item \verb+eqsbmgr_alloc+ --
\item \verb+eqsbmgr_realloc+ --
\item \verb+eqsbmgr_free+ --
\end{itemize}

Można też z dużym prawdopodobieństwem stwierdzić, że zwalniany blok pamięci
znajduje się już w jakiejś linii pamięci podręcznej.  Pożądaną sytuacją byłoby,
gdyby zaallokowany rekord bezpośrednio po oddaniu go na użytek programu był
dostępny w pamięci podręcznej.
...
Obszary o rozmiarze $> 32B$ i $\le 32KiB$.
